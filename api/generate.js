...
// ── Unsplash 큐레이션 이미지 (탭별 주제 쿼리) ────────────────────
// top: 1면 히어로용, section: 본문 중간 삽입용 (각 탭마다 여러 쿼리로 다양성 확보)
const UNSPLASH_QUERIES = {
  economy: {
    top:     ['korea money won finance', 'seoul bank economy', 'stock market chart korea'],
    section: ['coins money stack', 'financial calculator desk', 'office meeting business', 'bank interior'],
  },
  industry: {
    top:     ['semiconductor chip factory', 'korea manufacturing line', 'technology innovation lab'],
    section: ['electronics chip closeup', 'industrial machinery', 'tech office worker', 'automation robot'],
  },
  global: {
    top:     ['world map trade', 'international shipping port', 'global economy skyline'],
    section: ['airport cargo plane', 'shipping containers', 'currency exchange board', 'new york wall street'],
  },
  stocks: {
    top:     ['stock market trading floor', 'investment chart screen', 'candlestick graph'],
    section: ['financial newspaper', 'trader computer screens', 'bull bear market', 'coin stack portfolio'],
  },
};
...
// 탭 주제에 맞는 Unsplash 이미지 3장 (1 top + 2 section) 병렬 수집
async function fetchCuratedImages(tab) {
  const q = UNSPLASH_QUERIES[tab] || UNSPLASH_QUERIES.economy;
  const pick = arr => arr[Math.floor(Math.random() * arr.length)];
  const [top, s1, s2] = await Promise.all([
    fetchUnsplashImage(pick(q.top)),
    fetchUnsplashImage(pick(q.section)),
    fetchUnsplashImage(pick(q.section)),
  ]);
  return { top, sectionImages: [s1, s2].filter(Boolean) };
}
...
export default async function handler(req, res) {
  ...
  async function processTab(tab) {
    try {
      ...
      const { top, sectionImages } = await fetchCuratedImages(tab);
      const topImageUrl = top || sectionImages[0] || '';
